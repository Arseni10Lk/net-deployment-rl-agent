...
def load_midair_trajectory(hdf5_path: str, trajectory_name: str) -> TrajectoryData:
    ...
    with h5py.File(hdf5_path, "r") as f:
        ...
        quat = grp["attitude"][
            :
        ]  # Usually WXYZ or XYZW, MidAir specifies WXYZ typically, but we store it as is
...
if __name__ == "__main__":
    ...
    nb_traj = load_neurobem_csv(
        "dataset_data/neurobem/processed_data/merged_2021-02-03-13-43-38_seg_1.csv"
    )
    print(
        f"Loaded: {nb_traj.name} | Samples: {len(nb_traj.time)} | Duration: {nb_traj.time[-1]:.2f}s | dt: {nb_traj.dt:.4f}s"
    )

    print("\nTesting MidAir Loader...")
    ma_trajs = load_midair_all(
        "dataset_data/mid_air_data/MidAir/Kite_training/sunny/sensor_records.hdf5"
    )
    print(f"Loaded {len(ma_trajs)} trajectories from MidAir HDF5.")
    print(
        f"First traj: {ma_trajs[0].name} | Samples: {len(ma_trajs[0].time)} | Duration: {ma_trajs[0].time[-1]:.2f}s | dt: {ma_trajs[0].dt:.4f}s"
    )
