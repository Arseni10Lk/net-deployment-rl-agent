...
from net_interception_env.mechanics.guidance_algorithm.nominal_guidance import (
    compute_bgpn_acceleration,
)
from net_interception_env.mechanics.guidance_algorithm.cbf_filter import (
    compute_cbf_matrices,
)
...
class HOCBFGuidanceController:
    ...
    def compute_command(
        ...
        a_nom = compute_bgpn_acceleration(
            v_c=kin["v"], omega_los=kin["omega_los"], a_t=a_t, r=kin["r"], N=self.N
        )

        A_cbf, b_cbf = compute_cbf_matrices(
            r=kin["r"], v=kin["v"], a_t=a_t, d_min=self.d_min, k1=self.k1, k2=self.k2
        )
        ...
        try:
            a_cmd = qpsolvers.solve_qp(
                P=self.P, q=q, G=G, h=h, lb=self.lb, ub=self.ub, solver=self.solver
            )
            if a_cmd is None:
